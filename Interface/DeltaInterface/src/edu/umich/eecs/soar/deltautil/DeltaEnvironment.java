...
public abstract class DeltaEnvironment implements UpdateEventInterface/*, RunEventInterface*/ {
	// Fixed raw input space, x2 for t and t-1
	private ArrayList<GhostWME> dltaInputs1, 
								dltaInputs2;  // For efficiency, alternate which one is t and t-1
	
	private ArrayList<Rule> dltaDeltaRules;	// Stand-in for Rete elaboration rules that respond to input deltas
	private ArrayList<GhostWME> dltaInputElabs;	// WME symbols representing abstracted temporally-concurrent (TC) clusters of input WMES
	
	
	
	
	// OLD PROP3 STRUCTURES:
